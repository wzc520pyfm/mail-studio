...
import { componentDefinitions, createNode, predefinedSocialPlatforms } from '@/lib/mjml/schema';
import { PropSchema, EditorNode, MJMLComponentType } from '@/types/editor';
import { AlignLeft, AlignCenter, AlignRight, Trash2, Plus, GripVertical, ChevronDown, ChevronUp } from 'lucide-react';
      ...
      <ScrollArea className="flex-1">
        <div className="p-4 space-y-4">
          ...
          {selectedNode.content !== undefined && (
            <ContentEditor node={selectedNode} isHtmlContent={selectedNode.type === 'mj-table' || selectedNode.type === 'mj-raw'} />
          )}

          {/* Children Editor for components with child elements */}
          {hasChildrenEditor && (
            <ChildrenEditor node={selectedNode} />
          )}
...
function ContentEditor({ node, isHtmlContent = false }: { node: EditorNode; isHtmlContent?: boolean }) {
...
function ChildrenEditor({ node }: { node: EditorNode }) {
  const { updateNodeChildren, addChildNode, removeNode, setSelectedId } = useEditorStore();
